...
//   //origLog.call(this, "Log: " + str);
//   //origLog.call(this, " [" + chalk.red(new Date().getTime()) + "] " + str);

//   var logLine = highlight("[" + new Date().getTime() + "] " + str, { language: "shell"});
//   origLog.call(this, " " + logLine);
...
// }, 500)
function logFile() {
  var files = fs.readdir("./data/", (err, files) => {
    if (err) {
      console.error(err.message);
      console.error(err.stack);
      process.exit();
    } else if (files.length > 0) {
      fs.readFile(path.resolve("./data/" + files[Math.floor(Math.random() * files.length)]), async (err, data) => {
        if (err) {
          console.error(err.message);
          console.error(err.stack);
          process.exit();
...
for (var i=0;i<10;i++) {
  latencyData.x.push(' ');
  latencyData.y.push(Math.floor(Math.random() * 5));
}
